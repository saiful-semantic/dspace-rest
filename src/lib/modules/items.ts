...
import { internalBaseUrl, Payload, clientRequest } from '../client'
...
export const itemsFunctions = {
  ...
  all: (size: number = 20, page: number = 0): Promise<Items> =>
    clientRequest.get<Items>(`${ENDPOINTS.ITEMS}?size=${size}&page=${page}`),
   ...
   */
  byId: (itemId: string): Promise<Item> => clientRequest.get<Item>(`${ENDPOINTS.ITEMS}/${itemId}`),
  ...
  ): Promise<Item> => clientRequest.post<Item>(`${ENDPOINTS.COLLECTIONS}/${colId}/items`, payload),
  ...
  ): Promise<Item> => clientRequest.patch<Item>(`${ENDPOINTS.ITEMS}/${itemId}`, payload),
  ...
  deleteById: (itemId: string): Promise<void> =>
    clientRequest.delete<void>(`${ENDPOINTS.ITEMS}/${itemId}`),
